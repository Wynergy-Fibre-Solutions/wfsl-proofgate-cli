...
type VerifyResult =
  | { ok: true; reason: "verified" | string; details?: Record<string, unknown> }
...
/**
 * External verifier contract (STRICT, explicit opt-in):
 *
 * Provide --verifier <path-to-module>
 *
 * Module must export ONE of:
 *   - verifyExternalManifest(manifestPath: string, opts?: { publicKeyPath?: string }) => VerifyResult | Promise<VerifyResult>
 *   - verify(manifestPath: string, opts?: { publicKeyPath?: string }) => VerifyResult | Promise<VerifyResult>
 *   - default export with the same signature
 *
 * ProofGate will:
 *   - Load only when --verifier is provided and not "none"
 *   - Enforce function presence
 *   - Enforce VerifyResult shape
 *   - Fail deterministically with clear reasons
 */
async function tryExternalVerifier(
  ...
  if (!fs.existsSync(candidate)) {
    return {
      ok: false,
      reason: `external verifier not found at ${candidate}`
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      reason: `external verifier threw: ${String(e?.message ?? e)}`
    };
  }
}

/**
 * Built-in fallback verifier.
 *
 * Supported manifest fields:
 * {
 *   "messageFile": "./message.txt",
 *   "signatureFile": "./signature.b64" OR "signatureB64": "<base64>",
 *   "publicKeyFile": "./public.key.b64" OR "publicKeyB64": "<base64>"
 * }
 *
 * CLI also supports:
 *   --public-key <path>  (overrides manifest publicKeyFile/publicKeyB64)
 */
...
export async function runVerify(argv: string[]): Promise<void> {
  ...
  if (args.help) {
    process.stdout.write(
      [
        ...
        "",
        "External verifier (strict opt-in):",
        "  --verifier <path> loads your verifier module and enforces the contract.",
        "  --verifier none disables external verifier and forces builtin fallback.",
        "",
        "Manifest fields:",
        "  messageFile (required)",
        "  signatureFile OR signatureB64",
  ...
  if (external) {
    ...
    result = fallbackVerify(manifestPath, args.publicKeyPath);
  }
